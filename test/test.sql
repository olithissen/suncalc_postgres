        ...
        ASSERT (SELECT count(*)
                FROM (SELECT DISTINCT source.base_epoch,
                      ...
                      FROM sc_test_event source
                               LEFT JOIN get_sun_times(source.base_epoch, source.lat, source.lng,
                                                       source.height) pg_suncalc ON TRUE
                               LEFT JOIN sc_test_event source_event
                                         ON source.base_epoch = source_event.base_epoch AND
                                            source.lat = source_event.lat AND
                                            source.lng = source_event.lng AND source.height = source_event.height AND
                                            pg_suncalc.event = source_event.event_name) assert
                WHERE difference > 0.001) = 0;
    END;
$$;
        ...
        ASSERT (SELECT count(*)
                FROM (SELECT source.epoch,
                      ...
                      FROM sc_test_position source
                               LEFT JOIN get_sun_position(source.epoch, source.lat, source.lng) pg_suncalc
                                         ON TRUE) assert
                where difference_altitude > 0.00000008
                   OR difference_azimuth > 0.000002) = 0;
    END;
$$;